...
import com.dag.productmanagementservice.dao.SessionRepository;
import com.dag.productmanagementservice.dao.UserRepository;
import com.dag.productmanagementservice.dto.UserDTO;
...
class AuthServiceTest {
    ...
    void testLogOutSuccessful() {
        ...
        when(sessionRepository.findByTokenAndUserId(token, userId)).thenReturn(Optional.of(session));
        when(sessionRepository.save(any(Session.class))).thenAnswer(i -> i.getArgument(0));
    ...
    void testLogOutSessionNotFound() {
        ...
        when(sessionRepository.findByTokenAndUserId(token, userId)).thenReturn(Optional.empty());
